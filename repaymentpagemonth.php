...
$users = [];
$result = $conn->query("SELECT id, name FROM users_details_monthly");
...
if ($selectedDate) {
    ...
    $stmt = $conn->prepare("SELECT r.id, r.user_id, r.repayment_amount, r.repayment_date, u.name FROM repayment_history_monthly r JOIN users_details_monthly u ON r.user_id = u.id WHERE r.repayment_date = ?");
    $stmt->bind_param("s", $selectedDate);
    $stmt->execute();
    $result = $stmt->get_result();
    while ($row = $result->fetch_assoc()) {
        $repayments[] = $row;
        $totalRepayment += $row['repayment_amount']; // Ensure repayment_amount is a float
    }
    $stmt->close();
}
?>
...
<header class="text-white text-center  repayment-header" style="background-color: black;">
        <h2 class="text-white text-center py-2" >Monthly Finance</h2>
    </header>
    ...
    <div class="container pt-4">
        <div>
            <div class="text-center mb-3">
                <button type="button" class="btn btn-info btn-custom mx-2" onclick="window.location.href='monthly.php'">
                    <i class="fas fa-arrow-left"></i> Back to Month
                </button>
                <button type="button" class="btn btn-info btn-custom mx-2" onclick="window.location.href='index.php'">
                    <i class="fas fa-arrow-left"></i> Back to Main Page
                </button>
            </div>
            <form id="repaymentForm" method="post" action="repayment_monthly.php">
        ...
        <div class="d-flex repayment-collection">
            <?php if ($selectedDate) { ?>
                <div class="total-repayment">
                    <h4>Total Repayment for <?php echo htmlspecialchars($selectedDate); ?>:</h4>
                    <p><?php echo intval($totalRepayment); ?></p>
                </div>
            <?php } ?>
                    ...
                    <tbody>
                        ...
                        foreach ($repayments as $repayment) { ?>
                            <tr>
                                ...
                                <!-- Use number_format to display two decimal places -->
    ...
    <script>
        $(function () {
            ...
            $('#datePicker').on('change', function () {
                ...
                window.location.href = `repaymentpagemonth.php?date=${selectedDate}`;
            });

            // Handle form submission
            $('#repaymentForm').on('submit', function (event) {
                event.preventDefault();
                alert('Do You Want To Save This Transaction?');
            ...
            $('.delete-btn').on('click', function () {
                ...
                {
                    $.ajax({
                        url: 'delete_repayment_monthly.php',
                        type: 'POST',
                        data: { id: repaymentId },
                        success: function (response) {
                            const result = JSON.parse(response);
